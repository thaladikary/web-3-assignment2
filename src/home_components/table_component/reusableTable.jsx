...
const Table = ({ data,onDriverClick, onConstructorClick  }) => {
  ...
  const renderDriver = (driverObj, onClick) => {
    ...
    return (
      <span className="cursor-pointer hover:underline font-bold" onClick={() => onClick(driverObj.driverRef)}>
        {driverObj.driver}
      </span>
    );
  };
  ...
  const renderConstructor = (constructor, onClick) => {
    return (
      <span className="cursor-pointer hover:underline font-bold" onClick={() => onClick(constructor)}>
        {constructor} 
      </span>
    );
  };
